...
class EditFilasTriagem extends FormRequest
{
    /**
    ...
    public function rules()
    {
        ...
        $fila = Route::current()->parameters['fila'];
        ...
        return [
            ...
            'identificador' => [
                ...
                'required',
                'max:2',
                Rule::unique('filas_triagem')->where(function ($query) use ($instituicao, $fila) {
                    return $query->where('instituicoes_id', $instituicao->id)->where('identificador', '!=', $fila->identificador);
                })
            ],
            'ativo' => [
                'in:on',
