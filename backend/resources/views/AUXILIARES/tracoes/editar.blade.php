...
@section('conteudo')
    @component('components/page-title', [
        'titulo' => "Editar tração #{$tracao->id} {$tracao->descricao}",
        'breadcrumb' => [
    ...
    <div class="card">
        <div class="card-body">
            <form action="{{ route('tracoes.update', [$tracao]) }}" method="post">
                @method('put')
                @csrf

                <div class="form-group @if($errors->has('descricao')) has-danger @endif">
                    <label class="form-control-label">Descrição</span></label>
                    <input type="text" name="descricao" value="{{ old('descricao', $tracao->descricao) }}"
