...
@section('conteudo')
    ...
    <form @submit.prevent="submit" id='form' action="{{ route('instituicao.grupos.agenda.update', [$grupo_instituicao]) }}" method="post">
                @method('put')
                @csrf
    ...
    <div class="card card-outline-info" >
        <div class="card-header">
            <h4 class="m-b-0 text-white">{{$grupo_instituicao->grupo->nome}}</h4>
        </div>
        <div class="card-body">
            @foreach ($semana as $s)
                @php
                    $agenda = $grupo_instituicao->agenda()->where('dias_continuos',$s->value)->first();
                @endphp
                <div class='row @if($agenda || old('checkbox.'.$s->value)) panel @else no-panel @endif'>
                    ...
                    <div class="form-group col-sm-1 col-xs-12" style="text-align:center">
                        ...
                        <div >
                                <input type="checkbox" class='form-control checkbox' @if($agenda || old('checkbox.'.$s->value)) checked @endif value="{{$grupo_instituicao->id}}" id='checkbox[{{$s->value}}]' name="checkbox[{{$s->value}}]" >
                        </div>
                    </div>
...
<script src="{{ asset('material/assets/plugins/clockpicker/dist/jquery-clockpicker.min.js') }}"></script>
    ...
    <script>
        ...
        var vue = new Vue({
            ...
            data: {
                    @php
                        $agenda = $grupo_instituicao->agenda()->whereNotNull('dias_unicos')->first();
                    @endphp
            ...
            methods: {
                ...
                submit(){
                    ...
                    $.ajax("{{ route('instituicao.grupos.agenda.update', [$grupo_instituicao]) }}", {
                        method: "POST",
                        data: formData,
                        processData: false,
                        contentType: false,
