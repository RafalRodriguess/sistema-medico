...
@section('conteudo')
    @component('components/page-title', [
        'titulo' => "Editar saída de estoque #{$saida->id}",
        'breadcrumb' => [
    ...
    @endcomponent
    <div class="card">
        <form action="{{ route('instituicao.saidas-estoque.update', $saida) }}" method="post">
            <div class="card-body">
                @csrf
                @method('PUT')
                ...
                <div class="row">
                        ...
                        <select name="estoques_id" id="estoque-select" style="width: 100%" class="form-control">
                            ...
                            @foreach ($estoques as $estoque)
                                <option value="{{ $estoque->id }}"
                                    @if (old('estoques_id', $saida->estoques_id) == $estoque->id) selected="selected" @endif>{{ $estoque->descricao }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-4 form-group">
                        ...
                        <select name="tipo_destino" id="tipo_destino" class="form-control">
                            ...
                            @foreach (\App\SaidaEstoque::destino_saida as $id => $destino)
                                <option value="{{ $id }}" @if (old('tipo_destino', $saida->tipo_destino) == $id) selected @endif>
                                    {{ $destino }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-12 col-sm-10 form-group">
                        ...
                        <textarea name="observacoes" id="observacoes-input" rows="4" class="form-control">{{ old('observacoes', $saida->observacoes) }}</textarea>
                    </div>
                </div>
                <hr>
                <div class="row">
                ...
                <div class="row">
                    ...
                    <div id="pagamento-display" class="form-group col-md-3 col-sm" style="display: none">
                        <label for="display-pagamento-total">Valor restante a pagar
                            <i class="fa fa-question-circle help" data-toggle="tooltip" data-placement="right"
                                title=""
                    ...
                    <div class="input-group col-md col-sm-12 d-flex align-items-center">
                        <input type="checkbox" name="gerar_conta" id="check-gerar-conta-receber"
                            @if (!empty(old('gerar_conta', $saida->gerar_conta))) checked="checked" @endif>
                            ...
                            <select name="conta_id" id="conta_id" class="form-control select2-simple"
                                ...
                                @foreach ($contas_caixa as $conta)
                                    <option value="{{ $conta->id }}"
                                        @if (old('conta_id', $conta_caixa_selecionada->id) == $conta->id) selected @endif>
                            ...
                            <select name="plano_conta_id"
                                ...
                                @foreach ($planos_conta as $item)
                                    <option value="{{ $item->id }}"
                                        @if (old('plano_conta_id', $plano_conta_selecionado->id) == $item->id) selected="selected" @endif>{{ $item->codigo }}
