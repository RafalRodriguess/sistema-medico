...
@section('conteudo')
    ...
    @component('components/page-title', [
        'titulo' => 'Cadastrar Modelo de relatório',
        'breadcrumb' => [
            'Modelo de relatório' => route('instituicao.modeloRelatorio.index'),
            'Novo',
        ],
    ])
    @endcomponent
    ...
    <div class="card">
        <div class="card-body ">
            <form action="{{ route('instituicao.modeloRelatorio.store') }}" method="post">
                @csrf
               <div class="row">
                        ...
                        <select class="form-control select2 @if($errors->has('instituicao_prestador_id')) form-control-danger @endif" name="instituicao_prestador_id" id="instituicao_prestador_id" style="width: 100%">
                            @foreach ($prestadores as $item)
                                <option value="{{$item->id}}" @if (old('instituicao_prestador_id') == $item->id)
                                    selected
                                @endif>
                                    {{$item->prestador->nome}} ({{(!empty($item->especialidade)) ? $item->especialidade->descricao : ""}})
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="form-group text-right pb-2">
                    <a href="{{ route('instituicao.modeloRelatorio.index') }}">
