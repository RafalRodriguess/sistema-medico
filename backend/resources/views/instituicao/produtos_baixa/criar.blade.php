...
@section('conteudo')
    @component('components/page-title',
        [
            'titulo' => "Cadastrar produto na baixa de estoque #{$estoqueBaixa->id}",
            'breadcrumb' => [
                'Produtos da baixa de estoque' => route('instituicao.produtos_baixa.index', [$estoqueBaixa]),
                'Cadastrar produto na baixa de estoque',
            ],
        ])
    @endcomponent

    <div class="card">
        <form action="{{ route('instituicao.produtos_baixa.store', [$estoqueBaixa]) }}" method="POST">
            @csrf
            <div class="card-body">
                <div class="row">
                        ...
                        <select name="produto_id" class="form-control @error('produto_id') form-control-danger @enderror">
                            ...
                            @foreach ($produtos as $key => $item)
                                <option value="{{ $item->id }}"
                                    {{  $item->id == old('id') ? "selected='selected'" : '' }}>
                                    {{ $item->descricao }}
                                </option>
                            @endforeach
                        </select>
