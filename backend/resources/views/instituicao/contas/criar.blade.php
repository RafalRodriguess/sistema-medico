...
@section('conteudo')
    ...
    <div class="card">
        <div class="card-body ">
            <form action="{{ route('instituicao.contas.store') }}" method="post">
                @csrf
               <div class="row">
                    ...
                    <div class=" col-md-3 form-group @if($errors->has('tipo')) has-danger @endif">
                        ...
                        <select class="form-control @if($errors->has('tipo')) form-control-danger @endif" name="tipo" id="tipo" required>
                            @foreach ($opcoes_tipo as $id => $opcao)
                                <option value="{{ $id }}" @if(old('tipo', 1) == $id) selected="selected" @endif>{{ $opcao }}</option>
                            @endforeach
                        </select>
                        @if($errors->has('tipo'))
                    ...
                    <div class=" col-md-3 form-group @if($errors->has('saldo_inicial')) has-danger @endif">
                        ...
                        <input type="text" alt='decimal' name="saldo_inicial" value="{{ old('saldo_inicial') }}"
                ...
                <div class="banco">
                    <div class="row">
                        <div class=" col-md-4 form-group @if($errors->has('banco')) has-danger @endif">
                            ...
                            class="form-control @if($errors->has('banco')) form-control-danger @endif">
                            @if($errors->has('descricao'))
