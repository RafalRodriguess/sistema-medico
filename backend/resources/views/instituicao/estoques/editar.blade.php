...
@section('conteudo')
    ...
    @component('components/page-title', [
        'titulo' => "Editar Estoque #{$estoque->id} {$estoque->descricao}",
        'breadcrumb' => [
            'Estoques' => route('instituicao.estoques.index'),
            'Atualização',
        ],
    ])
    @endcomponent
    ...
    <div class="card">
        <div class="card-body ">
            <form action="{{ route('instituicao.estoques.update', [$estoque]) }}" method="post">
                @method('put')
                @csrf
                ...
                <div class="row">
                    <div class=" col-md-12 form-group @if($errors->has('descricao')) has-danger @endif">
                        ...
                        <input type="text" name="descricao" value="{{ old('descricao', $estoque->descricao) }}"
                    ...
                    <div class=" col-md-5 form-group @if($errors->has('tipo')) has-danger @endif">
                        ...
                        <select class="form-control @if($errors->has('tipo')) form-control-danger @endif" name="tipo" id="tipo" required>
                            ...
                            <option value="estoque" @if(old('tipo', $estoque->tipo)=='estoque') selected @endif >Estoque</option>
                            <option value="sub_estoque" @if(old('tipo', $estoque->tipo)=='sub_estoque') selected @endif >Sub Estoque</option>
                        </select>
                        @if($errors->has('tipo'))
                    ...
                    <div class=" col-md-7 form-group @if($errors->has('centro_custo_id')) has-danger @endif">
                        ...
                        <select class="form-control select2 @if($errors->has('centro_custo_id')) form-control-danger @endif" name="centro_custo_id" id="centro_custo_id" required style="width: 100%">
                            ...
                            @foreach($centro_custos as $centro_custo)
                                <option value="{{ $centro_custo->id }}" @if(old('tipo', $estoque->centro_custo_id)==$centro_custo->id) selected @endif >{{ $centro_custo->descricao }}</option>
                            @endforeach
                        </select>
