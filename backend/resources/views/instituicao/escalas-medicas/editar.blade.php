...
@section('conteudo')
    ...
    <div class="card">
        <div class="card-body ">
            <form action="{{ route('instituicao.escalas-medicas.update', [$escala_medica]) }}" method="post">
                @method('put')
                @csrf
                ...
                <div class="row">
                    <div class="col-md-10 form-group @if($errors->has('regra')) has-danger @endif">
                        ...
                        <input type="text" name="regra" class="form-control"
                            value="{{ old('regra', $escala_medica->regra) }}">
                        @if($errors->has('regra'))
                    ...
                    <div class="col-md-2 form-group @if($errors->has('data')) has-danger @endif">
                        ...
                        <input type="date" name="data" class="form-control"
                            value="{{ old('data', $escala_medica->data) }}">
                        @if($errors->has('data'))
                        ...
                        <select class="form-control p-0 m-0"
                            ...
                            @foreach ($especialidades as $especialidade)
                                <option value="{{ $especialidade->id }}"
                                    @if ($escala_medica->especialidade->id==$especialidade->id)
                                        selected
                    ...
                    <div class="col-md-4 form-group @if($errors->has('origem_id')) has-danger @endif">
                        ...
                        <select class="form-control p-0 m-0" name="origem_id">
                            ...
                            @foreach ($origens as $origem)
                                ...
                                @if ($escala_medica->origem->id==$origem->id)
                                    selected
                                @endif @if (old('origem_id')==$origem->id)
                                    selected
                ...
                <div class="col-sm-12 p-0 m-0 mb-3">
                    ...
                    @if($errors->has('prestadores'))
                        <div class="row p-0 m-0 pt-2 pb-2" id="prestadores-errors">
                            <small class="form-control-feedback">{{ $errors->first('prestadores') }}</small>
                        </div>
                    @endif

                    <div class="row p-0 m-0 pt-2 pb-2" id="prestadores-lista">
                        @if (old('prestadores', false))
                        ...
                        @foreach ($escala_medica->escalaPrestadores as $i => $prestador)
                            @php
                                $especialidades = $prestador->prestadorInstituicaoByInstituicao($instituicao->id)->especialidades()->get();
                            @endphp
                            <div class="card col-sm-12 shadow-none prestador-item p-0 m-0 mb-2 mt-1" id="{{ $i }}">
                                <div class="row bg-light border-bottom d-flex justify-content-end p-0 m-0">
                                    <div class="col d-flex p-2 m-0">
                                        <div class="row col-sm-12 d-flex justify-content-between align-self-center p-0 m-0">
                                            <span class="text-dark">
                                                #{{ $i + 1 }}
                                                {{-- {{ App\Especialidade::find($escala_medica->escalaPrestadores[$i]->id)->nome }} #{{ $i }} --}}
                                            </span>
                                            <button onclick="$(this).parent().parent().parent().parent().remove();"
                                                type="button" class="btn btn-danger">
                                                <i class="ti-close"></i>
                                ...
                                <div class="row p-1 m-0">
                                    <div class="col-md-4 form-group">
                                        ...
                                        <select readonly class="disabled form-control p-0 m-0 select2Live old-prestadores"
                                            data-prev="{{ $prestador->id }}"
                                            name="prestadores[{{$i}}][prestador_id]">
                                            <option selected value="{{ $prestador->id }}">{{ $prestador->nome }}</option>
                                        </select>
                                    </div>
                                    <div class="col-md-2 form-group">
                                        <label class="form-control-label p-0 m-0">Horário de Entrada <span class="text-danger">*</span></label>
                                        <input type="time" class="form-control"
                                            name="prestadores[{{$i}}][entrada]" value="{{ $prestador->pivot->entrada }}">
                                    </div>
                                    <div class="col-md-2 form-group">
                                        <label class="form-control-label p-0 m-0">Horário de Saída <span class="text-danger">*</span></label>
                                        <input type="time" class="form-control"
                                            name="prestadores[{{$i}}][saida]" value="{{ $prestador->pivot->saida }}">
                                    </div>
                                    <div class="col-md-4 form-group">
                                        <label class="form-control-label p-0 m-0">Observação</label>
                                        <input type="text" class="form-control"
                                            name="prestadores[{{$i}}][observacao]" value="{{ $prestador->pivot->observacao }}">
                                    </div>
                                </div>
                            </div>
                        @endforeach
