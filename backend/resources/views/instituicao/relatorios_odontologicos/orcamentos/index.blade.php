...
@section('conteudo')
    ...
    <div class="row page-titles">
        <div class="col-md-5 col-8 align-self-center">
            <h3 class="text-themecolor m-b-0 m-t-0">Orçamentos</h3>
            <ol class="breadcrumb">
    ...
    <div class="row">
        <div class="col-12">
            ...
            <div class="card">
                <div class="card-body">
                    <form action="javascript:void(0)" class="formRelatorioOrcamentos" id="formRelatorioOrcamentos">
                        @csrf
                        <div class="row">
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    <label class="form-label">Negociadores</label>
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    <label class="form-label">Avaliadores</label>
                                    <select class="form-control select2" style="width: 100%" name="avaliadores[]" id="avaliadores" multiple>
                                        <option value="" selected>Todos</option>
                                        @foreach ($negociadores as $item)
                                            <option value="{{$item->id}}" >{{$item->nome}}</option>
                                        @endforeach
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    ...
                                    <select class="form-control select2" style="width: 100%" name="responsaveis[]" id="responsaveis" multiple>
                                        <option value="" selected>Todos</option>
                                        @foreach ($negociadores as $item)
                                            <option value="{{$item->id}}" >{{$item->nome}}</option>
                                        @endforeach
                                    </select>
                                    <span style="cursor: pointer" onclick="limpa_filtros('responsaveis')" class="help" data-toggle="tooltip" data-placement="top" data-original-title="Limpar filtros"><i class="fa fa-trash"></i> </span>
                                    <span style="cursor: pointer" onclick="seleciona_filtros('responsaveis')" class="help" data-toggle="tooltip" data-placement="top" data-original-title="Selecionar todos os filtros"><i class="fa fa-reply-all"></i> </span>
...
@push('scripts')
    <script>
        ...
        $('#formRelatorioOrcamentos').on('submit', function(e){
            e.preventDefault()
            var formData = new FormData($(this)[0]);
            $.ajax("{{route('instituicao.relatorioOrcamentos.tabela')}}", {
                method: "POST",
                data: formData,
                processData: false,
                contentType: false,
