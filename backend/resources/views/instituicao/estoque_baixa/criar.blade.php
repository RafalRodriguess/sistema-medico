...
@section('conteudo')
    @component('components/page-title', [
        'titulo' => 'Cadastrar baixa de estoque',
        'breadcrumb' => [
    ...
    <div class="card" style="padding: 20px;">
        <form action="{{ route('instituicao.estoque_baixa_produtos.store') }}" method="POST">
            @csrf
            <div class="row">
                <div class="col-sm-6">
                        ...
                        <select name="estoque_id"
                            ...
                            @foreach ($estoques as $estoque)
                                <option value="{{ $estoque->id }}"
                                    @if (old('estoque_id') == $estoque->id) selected="selected" @endif>
                                    {{ $estoque->descricao }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <select name="motivo_baixa_id"
                            ...
                            @foreach ($motivos as $motivo)
                                <option value="{{ $motivo->id }}"
                                    @if (old('motivo_baixa_id') == $motivo->id) selected="selected" @endif>
                                    {{ $motivo->descricao }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <select name="setor_id"
                            ...
                            @foreach ($setores_exame as $setor)
                                <option value="{{ $setor->id }}"
                                    @if (old('setor_id') == $setor->id) selected="selected" @endif>
                                    {{ $setor->descricao }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="px-3">
                    <div class="form-group @if ($errors->has('data_hora_baixa')) has-danger @endif">
                        ...
                        <input type="time" name="data_hora_baixa" value="{{ old('data_hora_baixa', date('H:i:s')) }}"
