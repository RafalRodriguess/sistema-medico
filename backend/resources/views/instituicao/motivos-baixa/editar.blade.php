...
@section('conteudo')
    @component('components/page-title', [
        'titulo' => "Editar motivos para baixa de estoque #{$motivo->id}",
        'breadcrumb' => [
            'Motivos para baixa de estoque' => route('instituicao.motivos-baixa.index'),
            'Editar motivos para baixa de estoque',
        ],
    ])
    @endcomponent
    ...
    <div class="card">
        <div class="card-body ">
            <form action="{{ route('instituicao.motivos-baixa.update', $motivo) }}" method="post">
                @method('put')
                @csrf
                ...
                <div class="row">
                    <div class=" col-md-8 form-group @if ($errors->has('descricao')) has-danger @endif">
                        ...
                        <input rows="3" type="text" name="descricao"
                            class="form-control @if ($errors->has('descricao')) form-control-danger @endif" value="{{ old('descricao', $motivo->descricao) }}"/>
