<div id="modalReceberConta" class="modal fade bs-example-modal-lg" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="display: none;">
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">Receber parcela #{{$conta->id}}</h4>
            ...
            <form id="formReceberParcela">
                ...
                <div class="modal-body">
                    ...
                    <div class="row">
                        <div class="form-group col-md-3" @if ($conta->status == 1) style="display: none" @endif>
                            ...
                            <select name="conta_id" id="conta_id" class="form-control select2pagar " style="width: 100%" @if ($conta->status == 1) readonly @endif>
                        ...
                        <div class="form-group col-md-3">
                            <label for="forma_pagamento_input" class="control-label">Forma de recebimento:</label>
                            <select class="form-control select2Pagar" name="forma_pagamento" id="forma_pagamento_input" style="width: 100%">
                                <option value="">Selecione</option>
                                @foreach ($formas_pagamento as $item)
                                    <option value="{{$item}}" @if (old('forma_pagamento', $conta->forma_pagamento) == $item) selected="selected" @endif>
                                        {{ App\ContaReceber::forma_pagamento_texto($item)}}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="form-group col-md-3">
                            <label for="valor_pagar_parcela_pagar" class="control-label">Valor a receber:</label>
                        ...
                        <div class="form-group col-md-3">
                            ...
                            <input type="text" alt="decimal" class="form-control" id="valor_pagar_parcela" name="valor_pago" value="{{($conta->status == 0) ? $conta->valor_parcela : $conta->valor_pago}}" required>
                        </div>
                        ...
                        <div class="form-group col-md-3">
                            ...
                            <input type="date" class="form-control" name="data_pago" id="data_pagamento_parcela" @if ($conta->status == 1)
                                readonly
                            @endif value="{{date('Y-m-d')}}">
                        </div>
                        <div class="form-group col-sm-3">
                            <label for="parcela_compensacao_pagar" class="control-label">Data compensação:</label>
                            <input type="date" class="form-control" name="data_compensacao" id="parcela_compensacao_pagar" value="{{$conta->data_compensacao}}">
                        </div>

                        <div class="form-group col-sm-12">
                            <label for="parcela_obs_pagar" class="control-label">Observação:</label>
                            <textarea class="form-control" id="parcela_obs_pagar" name="obs">{{$conta->obs}}</textarea>
                        </div>

                        <div class="form-group col-sm-12 pagar_menor bg-danger text-white" style="display: none;">
                            <span class=''>O valor pago esta menor que o valor da parcela! deseja confirmar o recebimento a menor com desconto ou gerar uma nova parcela com a diferenca para um novo vencimento?</span>
                        </div>

                        <div class="form-group col-md-12 pagar_menor" style="display: none;">
                            <input type="checkbox" id="pagar_menor" name="pagar_menor" value="1" class="filled-in" />
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit" class="btn btn-danger waves-effect waves-light salvar_pagamento_parcela">receber</button>
                </div>
            </form>
        </div>
    </div>
...
<script>
    ...
    $("#formReceberParcela").on('submit', function(e){
        e.preventDefault()

        var formData = new FormData($(this)[0]);
        var id = $('#parcela_id_pagar').val();
        
        $.ajax({
            url: "{{ route('instituicao.contasReceber.receberParcela', ['contaReceber' => 'contaReceberId']) }}".replace('contaReceberId', id),
            method: "POST",
            data: formData,
            processData: false,
            contentType: false,
            success: function (result) {
                ...
                $(".id_"+id).css('background', '#54f75440');
                $(".status_"+id).text('pago');
            },
