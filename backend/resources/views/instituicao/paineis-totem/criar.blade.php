...
@section('conteudo')
    ...
    <div class="card">
        <div class="card-body ">
            <form action="{{ route('instituicao.totens.paineis.store') }}" method="post">
                @csrf
                <div class="row">
                    ...
                    <div class=" col-md-6 form-group @if($errors->has('origens_id')) has-danger @endif">
                        ...
                        class="form-control @if($errors->has('origens_id')) form-control-danger @endif">
                            @foreach ($origens as $origem)
                                <option value="{{ $origem->id }}" @if(old('origens_id') == $origem->id) selected @endif>{{ $origem->descricao }}</option>
                            @endforeach
                        </select>
                                    ...
                                    <tbody>
                                        @foreach ($opcoes as $key => $opcao)
                                            <tr>
                                                ...
                                                <td><input type="checkbox" name="opcoes[{{ $key }}][ativo]" class="checkbox-opcao" @if(!empty(old("opcoes.{$key}.ativo"))) checked @endif> {{ $opcao->descricao }}</td>
                                                <td>
                                                    <div class="d-flex">
                                                        <span class="text-danger mr-2">*</span> <input type="text" name="opcoes[{{ $key }}][titulo]" class="form-control" value="{{ old("opcoes.{$key}.titulo") }}">
                                                    </div>
                                                </td>
                                                <td>
                                                    <div class="d-flex">
                                                        <span class="text-danger mr-2">*</span> <input type="text" name="opcoes[{{ $key }}][local]" class="form-control" value="{{ old("opcoes.{$key}.local") }}">
                                                    </div>
                                                </td>
                                            </tr>
                                        @endforeach
