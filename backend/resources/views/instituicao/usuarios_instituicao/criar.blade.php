...
@section('conteudo')
    @component('components/page-title', [
        'titulo' => 'Cadastrar Usuários Instituicao',
        'breadcrumb' => [
    ...
    <div class="card">
        <div class="card-body">
            <form action="{{ route('instituicao.instituicoes_usuarios.store') }}" method="post" enctype="multipart/form-data">
                @csrf
                ...
                <div class="form-group @if($errors->has('cpf')) has-danger @endif">
                    ...
                    <input type="text" name="cpf" alt="cpf" id="cpf" value="{{ old('cpf') }}"
                ...
                <div class="form-group @if($errors->has('email')) has-danger @endif">
                    ...
                    <input type="email" name="email" name="example-email" id="email" value="{{ old('email') }}"
                ...
                <div class="form-group @if($errors->has('password')) has-danger @endif">
                    <label class="form-control-label senha">Senha *</label>
                ...
                <div class="form-group @if($errors->has('desconto_maximo')) has-danger @endif">
                    ...
                    <input type="decimal" name="desconto_maximo" id="desconto_maximo" value="{{ old('desconto_maximo') }}"
                ...
                {{-- <div class="form-group @error('instituicao_id') has-danger @enderror">
                    <label class="form-control-label">Instituicao</label>
                    <select name="instituicao_id"
                        class="form-control @error('instituicao_id') form-control-danger @enderror">
                        @foreach ($instituicao as $instituicoes)
                            <option value="{{ $instituicoes->id }}"
                                @if(old('instituicao_id') == $instituicoes->id) selected="selected" @endif>
    ...
    <script>
        $('#cpf').on('blur', function(){
            ...
            $.ajax({
                type: "GET",
                data: {cpf: cpf},
                url: "{{route('instituicao.verificaCpfExistenteInstituicao')}}",
                datatype: "json",
                success: function(result) {
                    ...
                    if(result.length > 0){
                        ...
                        $('#usuario_id').val(result[0].id)
                        $('.senha').text('Senha')
                        // var imagenUrl = result[0].foto;
                        // $("#image").attr("data-default-file", imagenUrl);
                        // $('.dropify').dropify();
                        // console.log(imagenUrl)
                        // $('#imagem').attr("data-default-file","{{ \Storage::cloud()->url("+result[0].foto+") }}")
                    }else{
                        $('#nome').val('')
                        $('#email').val('')
                        $('#usuario_id').val('')
        ...
        $( document ).ready(function() {
            ...
            $("form").submit(function(e){
                ...
                $.ajax("{{ route('instituicao.instituicoes_usuarios.store') }}", {
                    method: "POST",
                    data: formData,
                    processData: false,
                    contentType: false,
