...
@section('conteudo')
    ...
    @component('components/page-title', [
        'titulo' => "Editar Acomodação #{$acomodacao->id}",
        'breadcrumb' => [
            'Acomodação' => route('instituicao.internacao.acomodacoes.index'),
            'Atualizar',
        ],
    ])
    @endcomponent
    ...
    <div class="card">
        <div class="card-body ">
            <form action="{{ route('instituicao.internacao.acomodacoes.update', [$acomodacao]) }}" method="post">
                @method('put')
                @csrf
                ...
                <div class="row">
                    <div wire:ignore class="col-md-6 form-group @if($errors->has('descricao')) has-danger @endif">
                        ...
                        <input type="text" class="form-control" name="descricao"
                            value="{{ old('descricao', $acomodacao->descricao) }}">
                    ...
                    <div class="col-md-6 form-group @if($errors->has('tipo_id')) has-danger @endif">
                        ...
                        <select class="form-control p-0 m-0" name="tipo_id">
                            ...
                            @foreach ($tipos_acomodacoes as $tipo_id)
                                <option value="{{ $tipo_id }}" @if ($acomodacao->tipo_id==$tipo_id)
                                    selected
                                @endif @if (old('tipo_id')==$tipo_id)
                                    selected
                    ...
                    <div class="ml-auto col-sm-6 p-0 pt-4 m-0">
                        <div class="form-check">
                            <input type="checkbox" class="form-check-input" name="extra_virtual" value="1"
                                @if ($acomodacao->extra_virtual=="1") checked @endif
