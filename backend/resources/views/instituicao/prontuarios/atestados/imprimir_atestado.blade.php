...
<html lang="en">
    ...
    <body>
        <div class="print-table" @if (array_key_exists('impressao', $atestado->atestado))
            @if ($atestado->atestado['impressao'] != null)
            style="font-size: {{$atestado->atestado['impressao']['tamanho_fonte']}}px; padding: {{$atestado->atestado['impressao']['margem_cabecalho']}}cm {{$atestado->atestado['impressao']['margem_direita']}}cm {{$atestado->atestado['impressao']['margem_rodape']}}cm {{$atestado->atestado['impressao']['margem_esquerda']}}cm"
            @endif
            @endif>
            <div class="content-wrap">
                <div>
                    @if (array_key_exists('impressao', $atestado->atestado))
                        @if ($atestado->atestado['impressao'] != null)
                            {!! $atestado->atestado['impressao']['cabecalho'] !!}
                        @endif
                    @endif
                </div>
                <div >
                    ...
                    @if ($exibir_titulo_paciente)
                        <p style="text-align: center"><b><span>ATESTADO</span></b></p>
                        <p>
                            <div class="row">
                                <div class="col-md-12">
            ...
            <div id="footer">
                @if (array_key_exists('impressao', $atestado->atestado))
                    @if ($atestado->atestado['impressao'] != null)
                        {!! $atestado->atestado['impressao']['rodape'] !!}
                    @endif
                @endif
            </div>
        </div>
