...
<html lang="en">
    ...
    <body>
        <div class="print-table" @if (array_key_exists('impressao', $conclusao->conclusao))
            @if ($conclusao->conclusao['impressao'] != null)
            style="font-size: {{$conclusao->conclusao['impressao']['tamanho_fonte']}}px; padding: {{$conclusao->conclusao['impressao']['margem_cabecalho']}}cm {{$conclusao->conclusao['impressao']['margem_direita']}}cm {{$conclusao->conclusao['impressao']['margem_rodape']}}cm {{$conclusao->conclusao['impressao']['margem_esquerda']}}cm"
            @endif
            @endif>
            <div class="content-wrap">
                <div>
                    @if (array_key_exists('impressao', $conclusao->conclusao))
                        @if ($conclusao->conclusao['impressao'] != null)
                            {!! $conclusao->conclusao['impressao']['cabecalho'] !!}
                        @endif
                    @endif
                </div>
                <div >
                    ...
                    @if ($exibir_titulo_paciente)
                        <p style="text-align: center"><b><span>CONCLUSÃO</span></b></p>
                        <p>
                            <div class="row">
                                <div class="col-md-12">
                    ...
                    <p>
                        <div style="margin-top: 30px;">
                            Motivo: {{$conclusao->motivo->descricao}}
                        </div>
                    </p>
                    <p>
                        <div style="margin-top: 30px;">
                            {!!$conclusao->conclusao['obs']!!}
                        </div>
                    </p>
                </div>   
            </div>   
            ...
            <div id="footer">
                @if (array_key_exists('impressao', $conclusao->conclusao))
                    @if ($conclusao->conclusao['impressao'] != null)
                        {!! $conclusao->conclusao['impressao']['rodape'] !!}
                    @endif
                @endif
            </div>
        </div>
