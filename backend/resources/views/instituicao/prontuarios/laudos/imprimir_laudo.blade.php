...
<html lang="en">
    ...
    <body>
        <div class="print-table" @if (array_key_exists('impressao', $laudo->laudo))
            @if ($laudo->laudo['impressao'] != null)
            style="font-size: {{$laudo->laudo['impressao']['tamanho_fonte']}}px; padding: {{$laudo->laudo['impressao']['margem_cabecalho']}}cm {{$laudo->laudo['impressao']['margem_direita']}}cm {{$laudo->laudo['impressao']['margem_rodape']}}cm {{$laudo->laudo['impressao']['margem_esquerda']}}cm"
            @endif
            @endif>
            <div class="content-wrap">
                <div>
                    @if (array_key_exists('impressao', $laudo->laudo))
                        @if ($laudo->laudo['impressao'] != null)
                            {!! $laudo->laudo['impressao']['cabecalho'] !!}
                        @endif
                    @endif
                </div>
                <div >
                    ...
                    @if ($exibir_titulo_paciente)
                        <p style="text-align: center"><b><span>Laudo</span></b></p>
                        <p>
                            <div class="row">
                                <div class="col-md-12">
