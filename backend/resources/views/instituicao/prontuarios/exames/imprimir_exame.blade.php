...
<html lang="en">
    ...
    <body>
        <div class="print-table" @if (array_key_exists('impressao', $exame->exame))
            @if ($exame->exame['impressao'] != null)
            style="font-size: {{$exame->exame['impressao']['tamanho_fonte']}}px; padding: {{$exame->exame['impressao']['margem_cabecalho']}}cm {{$exame->exame['impressao']['margem_direita']}}cm {{$exame->exame['impressao']['margem_rodape']}}cm {{$exame->exame['impressao']['margem_esquerda']}}cm"
            @endif
            @endif>
            <div class="content-wrap">
                <div>
                    @if (array_key_exists('impressao', $exame->exame))
                        @if ($exame->exame['impressao'] != null)
                            {!! $exame->exame['impressao']['cabecalho'] !!}
                        @endif
                    @endif
                </div>
                <div >
                    ...
                    @if ($exibir_titulo_paciente)
                        <p style="text-align: center"><b><span>EXAME</span></b></p>
                        <p>
                            <div class="row">
                                <div class="col-md-12">
