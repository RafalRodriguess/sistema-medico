...
<html lang="en">
    ...
    <body>
        <div class="print-table" @if (array_key_exists('impressao', $encaminhamento->encaminhamento))
            @if ($encaminhamento->encaminhamento['impressao'] != null)
            style="font-size: {{$encaminhamento->encaminhamento['impressao']['tamanho_fonte']}}px; padding: {{$encaminhamento->encaminhamento['impressao']['margem_cabecalho']}}cm {{$encaminhamento->encaminhamento['impressao']['margem_direita']}}cm {{$encaminhamento->encaminhamento['impressao']['margem_rodape']}}cm {{$encaminhamento->encaminhamento['impressao']['margem_esquerda']}}cm"
            @endif
            @endif>
            <div class="content-wrap">
                <div>
                    @if (array_key_exists('impressao', $encaminhamento->encaminhamento))
                        @if ($encaminhamento->encaminhamento['impressao'] != null)
                            {!! $encaminhamento->encaminhamento['impressao']['cabecalho'] !!}
                        @endif
                    @endif
                </div>
                <div >
                    ...
                    @if ($exibir_titulo_paciente)
                        <p style="text-align: center"><b><span>ENCAMINHAMENTO</span></b></p>
                        <p>
                            <div class="row">
                                <div class="col-md-12">
                    ...
                    <p>
                        <div style="margin-top: 30px;">
                            {!!$encaminhamento->encaminhamento['obs']!!}
                        </div>
                    </p>
                </div>   
            </div>   
            ...
            <div id="footer">
                @if (array_key_exists('impressao', $encaminhamento->encaminhamento))
                    @if ($encaminhamento->encaminhamento['impressao'] != null)
                        {!! $encaminhamento->encaminhamento['impressao']['rodape'] !!}
                    @endif
                @endif
            </div>
        </div>
