...
<div class="row">
    <div class="col-12">
        ...
        <div class="card novo_atendimento" style="display: none">
            
        </div>

        <div class="card lista_atendimento">
            <div class="card-body">
                                ...
                                <td>
                                    @can('habilidade_instituicao_sessao', 'editar_atendimento_paciente')
                                        {{-- <a href="{{ route('instituicao.caixasCirurgicos.edit', [$item]) }}"> --}}
                                                ...
                                                data-toggle="tooltip" data-placement="top" data-original-title="Editar" data-id="{{$atendimento->id}}" data-pessoa="{{$atendimento->paciente_id}}">
                                    ...
                                    @can('habilidade_instituicao_sessao', 'excluir_atendimento_paciente')
                                        {{-- <form action="{{ route('instituicao.caixasCirurgicos.destroy', [$pessoaPesquisa, $item]) }}" id="form_atendimento_delete" method="post" class="d-inline form-excluir-registro">
                                            ...
                                            data-toggle="tooltip" data-placement="top" data-original-title="Excluir" data-id="{{$atendimento->id}}" data-pessoa="{{$atendimento->paciente_id}}">
                                                    <i class="ti-trash"></i>
                                            </button>
                                        {{-- </form> --}}
                                    @endcan
...
<script>
    $(".editarAtendimento").on('click', function(e){
        ...
        var pessoa_id = $(this).attr('data-pessoa');
        $.ajax({
    ...
    $(".excluirAtendimento").on('click', function(e){
        ...
        Swal.fire({
            ...
            if(result.isConfirmed){
                $.ajax({
                    url: "{{route('instituicao.atendimentos_paciente.excluirAgendamento', ['pessoa' => 'pessoa_id', 'atendimento_paciente' => 'atendimento_paciente_id'])}}".replace('pessoa_id', pessoa_id).replace('atendimento_paciente_id', id),
                    type: 'POST',
