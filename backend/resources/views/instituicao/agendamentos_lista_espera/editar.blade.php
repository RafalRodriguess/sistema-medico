...
@section('conteudo')
    ...
    <div class="card">
        <div class="card-body">
            <form action="{{ route('instituicao.agendamentosListaEspera.update', [$agendamento]) }}" method="post" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 form-group @if($errors->has('paciente_id')) has-danger @endif">
                        ...
                        <select name="paciente_id" id="paciente_id" class="select2agenda form-control @if ($errors->has('paciente_id'))
                            form-control-danger
                        @endif">
                            <option value="{{$agendamento->paciente_id}}">{{$agendamento->pessoa->nome}}</option>
                        </select>
                    ...
                    <div class="col-md-4" style="margin-top: 30px">
                        <div class="row">
                            <div class="col-md-6 form-group">
                                <input type="radio" id="especialidade" name="tipo_espera" class="filled-in" value="1" @if ($agendamento->especialidade_id)
                                    checked
                                @endif/>
                                <label for="especialidade">Especialidade<label>
                            {{-- </div>
                            <div class="col-md-6 form-group"> --}}
                                <input type="radio" id="prestador" name="tipo_espera" class="filled-in" value="1" @if ($agendamento->prestador_id)
                                checked
                            @endif/>
                    ...
                    <div class="col-md-6 especialidade form-group @if($errors->has('especialidade_id')) has-danger @endif">
                        ...
                        @endif">
                            ...
                            @foreach ($especialidades as $item)
                                <option value="{{$item->id}}" @if ($agendamento->especialidade_id == $item->id)
                                    selected
                    ...
                    <div class="col-md-6 prestador form-group @if($errors->has('prestador_id')) has-danger @endif">
                        ...
                        @endif" style="width: 100%">
                            ...
                            @foreach ($prestadores as $item)
                                <option value="{{$item->id}}" @if ($agendamento->prestador_id == $item->id)
                                    selected
                    ...
                    <div class="col-md-6 form-group @if($errors->has('convenio_id')) has-danger @endif">
                        ...
                        @endif" style="width: 100%">
                            ...
                            @foreach ($convenios as $item)
                                <option value="{{$item->id}}" @if ($agendamento->convenio_id == $item->id)
                                    selected
                                @endif>{{$item->nome}}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-12 form-group @if($errors->has('obs')) has-danger @endif">
                        ...
                        <textarea name="obs" id="obs" cols="10" rows="5" class="form-control">{{$agendamento->obs}}</textarea>
                    </div>
