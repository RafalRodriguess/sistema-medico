...
@section('conteudo')
    @component('components/page-title',
        [
            'titulo' => 'Cadastrar inventário de estoque',
            'breadcrumb' => [
                'Inventários de estoque' => route('instituicao.estoque_inventario.index'),
                'Cadastrar inventário de estoque',
            ],
        ])
    @endcomponent
    ...
    <div class="card">
        <div class="card-body">
            <form action="{{ route('instituicao.estoque_inventario.store') }}" method="POST">
                @csrf
                <div class="row">
                    ...
                    <div class="col-md-4 col-sm-6">
                        <div class="form-group @error('estoque_id') has-danger @enderror">
                            ...
                            <select id="estoque_id" name="estoque_id" class="form-control @error('estoque_id') form-control-danger @enderror">
                                <option value="">Selecione um estoque</option>
                                @foreach ($estoques as $estoque)
                                    <option value="{{ $estoque->id }}" @if (old('estoque_id') == $estoque->id) selected @endif>
                                        {{ $estoque->descricao }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="col-md-4 col-sm-6">
                        <div class="form-group @error('tipo_contagem') has-danger @enderror">
                            ...
                            <select name="tipo_contagem"
                                ...
                                <option @if (old('tipo_contagem') == 'Geral do Estoque') selected @endif value="Geral do Estoque">Geral do
                                    Estoque</option>
                                <option @if (old('tipo_contagem') == 'Apenas Alguns Produtos') selected @endif value="Apenas Alguns Produtos">
                        ...
                        <div class="form-group @if ($errors->has('aberta')) has-danger @endif">
                            ...
                            <div class="form-check">
                                <input class="form-check-input" type="radio" name="aberta" value="1" id="aberta1"
                                    @if (old('aberta', 1) == 1) checked @endif>
                            ...
                            <div class="form-check">
                                <input class="form-check-input" type="radio" name="aberta" value="0" id="aberta2"
                                    @if (old('aberta', 1) == 0) checked @endif>
...
@push('scripts')
    ...
    <script>
        ...
        $(document).ready(() => {
            ...
            const produtos_antigos = Array.from({!! json_encode(old('produtos', [])) !!});
