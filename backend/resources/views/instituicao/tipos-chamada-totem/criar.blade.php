...
@section('conteudo')
    @component('components/page-title', [
        'titulo' => 'Cadastrar Tipo de chamada',
        'breadcrumb' => [
    ...
    <div class="card">
        <div class="card-body ">
            <form action="{{ route('instituicao.totens.tipos-chamada.store') }}" method="post">
                @csrf
                    ...
                    <div class=" col-md-6 form-group @if($errors->has('ganchos_id')) has-danger @endif">
                        ...
                        class="form-control @if($errors->has('ganchos_id')) form-control-danger @endif">
                            @foreach ($ganchos as $gancho)
                                <option value="{{ $gancho->id }}" @if(old('ganchos_id') == $gancho->id) selected="selected" @endif>{{ $gancho->descricao }}</option>
                            @endforeach
                        </select>
