<div class="card">
    <div class="card-header">
        <div class="input-group justify-content-between">
            <h3 class="d-block">Relatório das saídas de estoque
                <span class="post-tooltip" data-toggle="tooltip" data-placement="right" title="" data-original-title="Relatório gerado, clique nos itens abaixo para expandir e exibir as saídas de estoque que foram registradas para cada um dos produtos"><i class="fas fa-question-circle"></i></span>
            </h3>
            <h3 class="d-block ml-3">{{ (new \DateTime($data_inicio))->format('d/m/Y') }} @if ($data_inicio != $data_fim) - {{ (new \DateTime($data_fim))->format('d/m/Y') }} @endif</h3>
        </div>
        <div class="context-buttons mt-3">
            ...
            <a class="result-item border p-2 d-block" data-toggle="collapse"
                ...
                <div class="result-header">
                    <div class="row col-12 align-items-center">
                        <div class="col">
                            <h4 class="my-0 mr-3 my-2">Saída de estoque #{{ $resultado->id }}</h4>
                        </div>
                ...
                <div class="border bg-light mt-2 collapse" id="result-items-container-{{ $key }}">
                    <div class="result-expand">
                        ...
                        @if (!empty($resultado->produtosSaida))
                            @foreach ($resultado->produtosSaida as $produtoSaida)
                                @if (!empty($produtoSaida->entradaProduto))
                                    @php
                                        $estoqueEntradaProduto = $produtoSaida->entradaProduto;
                                    @endphp
                                    <div class="result-subitem">
                                        <div>{{ $estoqueEntradaProduto->produto->descricao }}</div>
                                        <div>{{ $estoqueEntradaProduto->lote }}</div>
                                        <div>
                                            {{ $produtoSaida->quantidade . ' ' . ($estoqueEntradaProduto->produto->unidade ? $estoqueEntradaProduto->produto->unidade->descricao : '') }}
                                        </div>
