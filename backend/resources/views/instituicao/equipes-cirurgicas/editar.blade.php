...
@section('conteudo')
    ...
    @component('components/page-title', [
        'titulo' => 'Editar Equipe Cirúrgica',
        'breadcrumb' => [
    ...
    <div class="card">
        <div class="card-body ">
            <form action="{{ route('instituicao.centros.equipes.update', [$equipe_cirurgica]) }}"      method="post">
                @method('put')
                @csrf
                ...
                <div class="row">
                    <div class="col-md-12 form-group @if($errors->has('descricao')) has-danger @endif">
                        ...
                        <input type="text" name="descricao" class="form-control" value="{{ old('descricao', $equipe_cirurgica->descricao) }}">
                    ...
                    <div class="row p-0 m-0 pt-2 pb-2" id="equipes-cirurgicas-prestadores-lista">
                                ...
                                <div id="prestadores" class="col-md-6 form-group @if($errors->has("prestadores.0.prestador_id")) has-danger @endif">
                                    ...
                                    <select class="form-control p-0 m-0" name="prestadores[0][prestador_id]">
                                        ...
                                        <option value="{{ $prestador->id }}"
                                            @if ($equipe_cirurgica->equipeCirurgicaPrestadores[0]->pivot->prestador_id == $prestador->id)
                                                selected
                                            @endif @if (old("prestadores.{0}.prestador_id") == $prestador->id)
                                            selected
                        ...
                        @if (old('prestadores'))
                            @for ($i = 1; $i < count(old('prestadores')); $i++)
                                <div class="card col-sm-12 shadow-none equipe-cirurgica-prestador-item p-0 m-0 mb-2 mt-1" id="{{ $i }}">
                                    <div class="row bg-light border-bottom d-flex
                                    justify-content-end p-2 m-0">
                            ...
                            @endfor
                        @else
                            @for ($i = 1; $i < count($equipe_cirurgica->equipeCirurgicaPrestadores); $i++)
                                <div class="card col-sm-12 shadow-none equipe-cirurgica-prestador-item p-0 m-0 mb-2 mt-1">
                                    <div class="row bg-light border-bottom d-flex justify-content-end p-0 m-0" id="{{ $i }}">
                                        <div class="col d-flex p-2 m-0">
                                            <div class="row col-sm-12 d-flex justify-content-between align-self-center p-0 m-0">
                                                <span class="text-dark prestador-titulo">
                                                    Prestador #{{ $i }}
                                    ...
                                    <div class="row p-1 m-0">
                                        <div id="prestadores" class="col-md-6 form-group">
                                            ...
                                            <select class="form-control p-0 m-0 campo tipo prestadores-options" name="prestadores[{{$i}}][tipo]">
                                                <option selected disabled>Selecione</option>
                                                @foreach ($tipos as $tipo)
                                                    <option value="{{ $tipo }}" @if ($equipe_cirurgica->equipeCirurgicaPrestadores[$i]->pivot->tipo==$tipo)
                                                        selected
                                                    @endif >{{ App\EquipeCirurgica::getTipoTexto($tipo) }}</option>
                                                @endforeach
                                            </select>
                                        ...
                                        <div id="prestadores" class="col-md-6 form-group">
                                            ...
                                            <select class="form-control p-0 m-0 campo prestador_id prestadores-options" name="prestadores[{{$i}}][prestador_id]">
                                                <option selected disabled>Selecione</option>
                                                @foreach ($prestadores as $prestador)
                                                    <option value="{{ $prestador->id }}" @if ($equipe_cirurgica->equipeCirurgicaPrestadores[$i]->pivot->prestador_id)
                                                        selected
                                                    @endif >{{ $prestador->nome }}</option>
                                                @endforeach
                                            </select>
