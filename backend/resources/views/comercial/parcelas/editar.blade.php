...
@section('conteudo')
    @component('components/page-title', [
        'titulo' => "Editar Parcelas #{$comercial->id} {$comercial->nome_fantasia}",
        'breadcrumb' => [
            'Parcelas' => route('comercial.parcelas.edit'),
            'Editar',
        ],
    ])
    @endcomponent
    ...
    <div class="row">
        <div class="col-6">
            <div class="card">
                <div class="card-body">
                    <form action="{{ route('comercial.parcelas.update') }}" method="post">
                    @method('put')
                    @csrf
        ...
        <div class="col-6">
            ...
            <p class="text-muted m-t-0"><code>A partir de 1 parcela: </code>Serão descontados <code>{{str_replace('.',',',$comercial->taxa_tectotum + 3.79)}}%</code> + <code>R$1,20</code> por venda no cartão</p>
            <p class="text-muted m-t-0"><code>A partir de 2 parcela: </code>Serão descontados <code>{{str_replace('.',',',$comercial->taxa_tectotum + 4.19)}}%</code> + <code>R$1,20</code> por venda no cartão</p>
            <p class="text-muted m-t-0"><code>A partir de 7 parcela: </code>Serão descontados <code>{{str_replace('.',',',$comercial->taxa_tectotum + 4.59)}}%</code> + <code>R$1,20</code> por venda no cartão</p>
            </div>
        </div>
        <div class="col-12">
            <div class="card">
    ...
    <script>
        $( document ).ready(function() {
            ...
            for (let index = 0; index < parcelas; index++) {
                ...
                var taxa = (valor * (({{$comercial->taxa_tectotum}} + taxa_pagarme) /100))  + 1.20
