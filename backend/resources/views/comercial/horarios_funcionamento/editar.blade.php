...
@section('conteudo')
    ...
    <div class="card">
        <div class="card-body">
            <form action="{{ route('comercial.horarios_funcionamento.update') }}" method="post" enctype="multipart/form-data">
                @method('put')
                @csrf

                @foreach ($horarios as $item)
                    <div class="col-md-12">
                        <h4>{{ App\HorarioFuncionamentoComercial::ConvertDiaSemana($item->dia_semana) }}</h4>
                        <hr>
                        <div class="row">
