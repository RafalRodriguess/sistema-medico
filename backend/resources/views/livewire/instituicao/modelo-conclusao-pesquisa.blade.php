<div class="card-body">
    ...
    <form action="javascript:void(0)" id="FormTitular">
        <div class="row">
            ...
            @can('habilidade_instituicao_sessao', 'cadastrar_modelo_conclusao')
                <div class="col-md-2">
                    <div class="form-group" style="margin-bottom: 0px !important;">
                        <a href="{{ route('instituicao.modeloConclusao.create') }}">
        ...
        <tbody>
            @foreach ($modelos as $modelo)
                <tr>
                    ...
                    <td>{{ $modelo->motivoConclusao->descricao }}</td>
                    <td>{{ $modelo->descricao }}</td>
                    {{-- <td>{{ $procedimento->via_administracao }}</td> --}}
                    <td>
                        @can('habilidade_instituicao_sessao', 'editar_modelo_conclusao')
                            <a href="{{ route('instituicao.modeloConclusao.edit', [$modelo]) }}">
                        ...
                        @endcan
                        @can('habilidade_instituicao_sessao', 'excluir_modelo_conclusao')
                            <form action="{{ route('instituicao.modeloConclusao.destroy', [$modelo]) }}" method="post" class="d-inline form-excluir-registro">
                                @method('delete')
                                @csrf
