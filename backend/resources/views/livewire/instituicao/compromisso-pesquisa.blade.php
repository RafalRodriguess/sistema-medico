<div class="card-body">
    ...
    <form action="javascript:void(0)" id="FormTitular">
        <div class="row">
            ...
            @can('habilidade_instituicao_sessao', 'cadastrar_compromissos')
                <div class="col-md-2">
                    <div class="form-group" style="margin-bottom: 0px !important;">
                        <a href="{{ route('instituicao.compromissos.create') }}">
        ...
        <tbody>
            @foreach ($compromissos as $item)
                <tr>
                    ...
                    <td><span style="background-color: {{ $item->cor }}; pointer-events: none; font-weight: bold;" class="btn">{{ strtoupper($item->cor) }}</span></td>
                    <td>{{ $item->descricao }}</td>
                    <td>
                        @can('habilidade_instituicao_sessao', 'editar_compromissos')
                            <a href="{{ route('instituicao.compromissos.edit', [$item]) }}">
                        ...
                        @endcan
                        @can('habilidade_instituicao_sessao', 'excluir_compromissos')
                            <form action="{{ route('instituicao.compromissos.destroy', [$item]) }}" method="post"
