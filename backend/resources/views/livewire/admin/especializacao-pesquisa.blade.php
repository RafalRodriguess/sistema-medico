<div class="card-body">
    ...
    <form action="javascript:void(0)" id="FormTitular">
        <div class="row">
              ...
              @can('habilidade_admin', 'cadastrar_especializacao')
                                                  <div class="col-md-2">
                <div class="form-group" style="margin-bottom: 0px !important;">
                     <a href="{{ route('especializacoes.create') }}">
        ...
        <tbody>
            @foreach($especializacoes as $especializacao)
                <tr>
                    ...
                    <td>
                        @can('habilidade_admin', 'editar_especializacao')
                            <a href="{{ route('especializacoes.edit', [$especializacao]) }}">
                                    <button type="button" class="btn btn-xs btn-secondary" aria-haspopup="true" aria-expanded="false">
                                            <i class="ti-pencil-alt"></i>
                                    </button>
                            </a>
                        @endcan
                        @can('habilidade_admin', 'excluir_especializacao')
                            <form action="{{ route('especializacoes.destroy', [$especializacao]) }}" method="post" class="d-inline form-excluir-registro">
                                @method('delete')
                                @csrf
