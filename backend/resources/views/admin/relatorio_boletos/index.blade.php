...
@section('conteudo')
    ...
    <div class="row page-titles">
        <div class="col-md-5 col-8 align-self-center">
            <h3 class="text-themecolor m-b-0 m-t-0">Boletos</h3>
            <ol class="breadcrumb">
    ...
    <div class="row">
        <div class="col-12">
            ...
            <div class="card">
                <div class="card-body">
                    <form action="javascript:void(0)" class="formRelatorioBoletos" id="formRelatorioBoletos">
                        @csrf
                        <div class="row">
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    <label class="form-label">Instituicoes</label>
                                    <select class="form-control select2" style="width: 100%" name="instituicoes[]" id="instituicoes" multiple>
                                        @foreach ($instituicoes as $item)
                                            <option value="{{$item->id}}" selected>{{$item->nome}}</option>
                                        @endforeach
                                    </select>
                                    <span style="cursor: pointer" onclick="limpa_filtros('instituicoes')" class="help" data-toggle="tooltip" data-placement="top" data-original-title="Limpar filtros"><i class="fa fa-trash"></i> </span>
                                    <span style="cursor: pointer" onclick="seleciona_filtros('instituicoes')" class="help" data-toggle="tooltip" data-placement="top" data-original-title="Selecionar todos os filtros"><i class="fa fa-reply-all"></i> </span>
...
@push('scripts')
    <script>
        ...
        $('#formRelatorioBoletos').on('submit', function(e){
            e.preventDefault()
            var formData = new FormData($(this)[0]);
            $.ajax("{{route('admin.relatorioBoletos.tabela')}}", {
                method: "POST",
                data: formData,
                processData: false,
                contentType: false,
