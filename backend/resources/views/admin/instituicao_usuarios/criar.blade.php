...
@section('conteudo')
    @component('components/page-title', [
        'titulo' => "Cadastrar Usuarios #{$instituicao->id} {$instituicao->nome}",
        'breadcrumb' => [
    ...
    <div class="card">
        <div class="card-body">
            <form action="{{ route('instituicao_usuarios.store', [$instituicao]) }}" method="post" enctype="multipart/form-data">
                @csrf
                    ...
                    <select name="instituicao_id"
                        ...
                        @foreach ($instituicao as $comerciais)
                            <option value="{{ $comerciais->id }}"
                                @if(old('instituicao_id') == $comerciais->id) selected="selected" @endif>
    ...
    <script>
        $('#cpf').on('blur', function(){
            ...
            $.ajax({
                type: "GET",
                data: {cpf: cpf},
                url: "{{route('verificaCpfExistenteInstituicao')}}",
                datatype: "json",
        ...
        $( document ).ready(function() {
            ...
            $("form").submit(function(e){
                ...
                $.ajax("{{ route('instituicao_usuarios.store', [$instituicao]) }}", {
                    method: "POST",
                    data: formData,
                    processData: false,
                    contentType: false,
                    success: function (response) {
                        ...
                        if(response.icon=="success"){
                            window.location="{{ route('instituicao_usuarios.index', [$instituicao]) }}";
                        }
                    },
