...
@section('conteudo')
    @component('components/page-title', [
        'titulo' => "Editar Administrador #{$administrador->id} {$administrador->nome}",
        'breadcrumb' => [
    ...
    <div class="card">
        <div class="card-body">
            <form action="{{ route('administradores.update', [$administrador]) }}" method="post" enctype="multipart/form-data">
                @method('put')
                @csrf

                <div class="form-group @if($errors->has('nome')) has-danger @endif">
                    <label class="form-control-label">Nome *</span></label>
                    <input type="text" name="nome" value="{{ old('nome', $administrador->nome) }}"
                ...
                <div class="form-group @if($errors->has('email')) has-danger @endif">
                    ...
                    <input type="email" name="email" name="example-email" value="{{ old('email', $administrador->email) }}"
                ...
                <div class="form-group @if($errors->has('cpf')) has-danger @endif">
                    ...
                    <input type="text" name="cpf" alt="cpf" value="{{ old('cpf', $administrador->cpf) }}"
                ...
                <div class="form-group @if($errors->has('imagem')) has-danger @endif">
                    ...
                    <label style="cursor: pointer;display:block;" data-toggle="tooltip" title="Foto" >
                            ...
                            @if ($administrador->foto)
                                src="{{ \Storage::cloud()->url($administrador->foto) }}"
                            @else
                ...
                <div class="form-group @if($errors->has('perfis_usuario_id')) has-danger @endif">
                    <label class="form-control-label">Perfil *</label>
                    <select name="perfis_usuario_id"
                        class="form-control @if($errors->has('perfis_usuario_id')) form-control-danger @endif">
                        @foreach ($perfis as $perfil)
                            <option value="{{ $perfil->id }}"
                                @if(old('perfis_usuario_id', $administrador->perfis_usuario_id) == $perfil->id) selected="selected" @endif>
                                {{ $perfil->descricao }}
                            </option>
                        @endforeach
                    </select>
                    @if($errors->has('perfis_usuario_id'))
                        <div class="form-control-feedback">{{ $errors->first('perfis_usuario_id') }}</div>
                    @endif
                </div>
    ...
    <script>
        ...
        $( document ).ready(function() {
            ...
            $("form").submit(function(e){
                ...
                $.ajax("{{ route('administradores.update', [$administrador]) }}", {
                    method: "POST",
                    data: formData,
                    processData: false,
                    contentType: false,
