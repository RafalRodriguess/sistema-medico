...
@section('conteudo')
    @component('components/page-title', [
        'titulo' => "Cadastrar Usuarios #{$comercial->id} {$comercial->nome_fantasia}",
        'breadcrumb' => [
            'Comercial' => route('comercial.index'),
            $comercial->nome_fantasia,
            'Usuarios' => route('comercial_usuarios.index', [$comercial]),
            'Novo',
        ],
    ])
    @endcomponent
    ...
    <div class="card">
        <div class="card-body">
            <form action="{{ route('comercial_usuarios.store', [$comercial]) }}" method="post" enctype="multipart/form-data">
                @csrf
                ...
                <div class="form-group @if($errors->has('email')) has-danger @endif">
                    ...
                    <input type="email" name="email" id="email" name="example-email" value="{{ old('email') }}"
                ...
                {{-- <div class="form-group @error('comercial_id') has-danger @enderror">
                    <label class="form-control-label">Comercial</label>
                    <select name="comercial_id"
                        class="form-control @error('comercial_id') form-control-danger @enderror">
                        @foreach ($comercial as $comerciais)
                            <option value="{{ $comerciais->id }}"
                                @if(old('comercial_id') == $comerciais->id) selected="selected" @endif>
    ...
    <script>
        ...
        $( document ).ready(function() {
            ...
            $("form").submit(function(e){
                ...
                $.ajax("{{ route('comercial_usuarios.store', [$comercial]) }}", {
                    method: "POST",
                    data: formData,
                    processData: false,
                    contentType: false,
                    success: function (response) {
                        ...
                        if(response.icon=="success"){
                            window.location="{{ route('comercial_usuarios.index', [$comercial]) }}";
                        }
                    },
        ...
        $('#cpf').on('blur', function(){
            ...
            $.ajax({
                type: "GET",
                data: {cpf: cpf},
                url: "{{route('verificaCpfExistente')}}",
                datatype: "json",
