...
@section('conteudo')
    @component('components/page-title', [
        'titulo' => "Editar habilidade #{$comercialUsuario->id} {$comercialUsuario->nome} #{$comercial->id} {$comercial->nome_fantasia}",
        'breadcrumb' => [
    ...
    <div class="card">
        <div class="card-body">
            <form action="{{ route('comercial_usuarios.habilidade', [$comercial, $comercialUsuario]) }}" method="post">
                @method('put')
                @csrf
                        ...
                        <div class="card">
                            <div class="card-body">
                                ...
                                <div class="ibox-content">
                                    ...
                                    @foreach ($hab_grupo->comercialHabilidades as $hab)
                                        @php($habilidade = $comercialUsuario->comercialHabilidades->where("nome_unico", $hab->nome_unico)->where("pivot.comercial_id", $comercial->id)->first())
                                        <div class="row habilidade" style="margin-bottom: 30px!important">
                                            ...
                                            <div class="demo-radio-button col-md-6">
                                                <div class="row">
                                                    <div class="col-md-6">
                                                        <input class="permitir" name="habilidades[{{ $hab->id }}]" type="radio" id="radio_{{ $hab->nome_unico }}_1"
                                                        @if ($habilidade)
                                                          @if ($habilidade->pivot->habilitado == '1')
                                                              checked
                                                          @endif
                                                        @endif
                                                        value="1" />
                                                    ...
                                                    <div class="col-md-6">
                                                        <input class="negar" name="habilidades[{{ $hab->id }}]" type="radio" id="radio_{{ $hab->nome_unico }}_2"
                                                        @if ($habilidade)
                                                          @if ($habilidade->pivot->habilitado == '0')
                                                              checked
                                                          @endif
                                                        @else
                                                          checked
                ...
                <div class="form-group text-right">
                        <button type="button" id="permitir_tudo" class='btn btn-primary'>Permitir Tudo</button>
                        <button type="button" id="negar_tudo" class='btn btn-primary'>Negar Tudo</button>
